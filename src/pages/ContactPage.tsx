        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 360px', gap: 52, alignItems: 'flex-start' }}>
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 'clamp(28px,4vw,40px)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                  ...
                  <div>
                    <Label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, fontWeight: 600, marginBottom: 6, display: 'block' }}>Email Address *</Label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                  ...
                  <div>
                    <Label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, fontWeight: 600, marginBottom: 6, display: 'block' }}>US State *</Label>
                    <select required value={form.state} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setForm(f => ({ ...f, state: e.target.value }))} style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: form.state ? '#fff' : 'rgba(255,255,255,0.4)', borderRadius: 10, height: 44, padding: '0 12px', fontSize: 14, outline: 'none' }}>
                      <option value="" disabled style={{ background: '#07091f' }}>Select your state</option>
                      {US_STATES.map(s => <option key={s} value={s} style={{ background: '#07091f', color: '#fff' }}>{s}</option>)}
                    </select>
                  </div>
                </div>
          ...
          <motion.div {...fadeUp(0.15)} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {[
              ...
              <motion.div key={i} {...fadeUp(0.1 * i)} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '20px 22px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  {item.link && <a href={item.link} target="_blank" rel="noreferrer" style={{ fontSize: 13, color: '#1b6fff', marginTop: 8, display: 'inline-block', textDecoration: 'none', fontWeight: 600 }}>{item.linkLabel}</a>}
                </div>
              </motion.div>
            ))}
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '20px 22px' }}>
              <div style={{ ...GS, fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 14 }}>What happens next?</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 10, alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', lineHeight: 1.5 }}>{step}</span>
                </div>
              ))}
            </div>
          </motion.div>
