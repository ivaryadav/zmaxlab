...
export default function ServicesPage() {
  ...
  const servicesSchema = [{
    ...
    "description": "Affordable custom healthcare website for NPI practitioners. Hand-coded, mobile-perfect, live in 7 days. $500 one-time flat fee.",
    "offers": {"@type": "Offer", "price": "500", "priceCurrency": "USD", "availability": "https://schema.org/InStock"},
    "areaServed": "United States"
  }]

  useSEO({
    title: 'Healthcare Website Design Services & Pricing | ZmaxLab',
    description: 'Custom healthcare website design for NPI practitioners. $500 flat fee, 7-day turnaround. Plus Local SEO ($230/mo) and social media ($150/mo). No contracts.',
    ...
    <div>
      <section style={{ padding: 'clamp(120px,14vw,160px) 5% clamp(64px,8vw,96px)', background: 'linear-gradient(155deg,#03051a,#07091f)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative' }}>
          <motion.div {...fadeUp()}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(0,200,150,0.1)', border: '1px solid rgba(0,200,150,0.2)', color: '#34d399', fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase' as const, padding: '5px 14px', borderRadius: 999, marginBottom: 20 }}>Transparent Pricing</span>
            <h1 style={{ ...GS, fontSize: 'clamp(2.2rem,5vw,4rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, letterSpacing: '-2px', marginBottom: 18 }}>Start with your website.<br/><span style={{ color: '#00c896' }}>Add growth when ready.</span></h1>
            <p style={{ fontSize: 'clamp(15px,1.5vw,18px)', color: 'rgba(255,255,255,0.6)', lineHeight: 1.75, maxWidth: 560, marginBottom: 28 }}>$500 gets your custom website live in 7 days. SEO, social media, and marketing are completely optional.</p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link to="/contact" style={{ ...GS, background: 'linear-gradient(135deg,#1b6fff,#00c896)', color: '#fff', fontWeight: 700, fontSize: 14, padding: '13px 28px', borderRadius: 999, boxShadow: '0 4px 20px rgba(27,111,255,0.3)', display: 'inline-flex', alignItems: 'center', gap: 8 }}>Get My Free Demo <ArrowRight size={15}/></Link>
              <Link to="/how-it-works" style={{ ...GS, border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.8)', fontWeight: 600, fontSize: 14, padding: '12px 24px', borderRadius: 999 }}>How It Works</Link>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <motion.div {...fadeUp(0.1)} style={{ background: 'linear-gradient(135deg,rgba(27,111,255,0.1),rgba(0,200,150,0.06))', border: '1px solid rgba(27,111,255,0.25)', borderRadius: 24, padding: 'clamp(28px,4vw,48px)', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 48, alignItems: 'start' }}>
            <div>
              ...
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.75, marginBottom: 28 }}>Every site coded from scratch for your specialty, location, and patient audience.</p>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28, textAlign: 'center', position: 'sticky', top: 96 }}>
              ...
              <div style={{ ...GS, fontSize: 68, fontWeight: 900, color: '#fff', lineHeight: 1, letterSpacing: '-4px' }}><sup style={{ fontSize: 26, verticalAlign: 'super', letterSpacing: 0 }}>$</sup>500</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 22 }}>one-time - yours forever</div>
              <Link to="/contact" style={{ ...GS, display: 'block', background: 'linear-gradient(135deg,#1b6fff,#00c896)', color: '#fff', fontWeight: 700, fontSize: 14, padding: '12px', borderRadius: 999, marginBottom: 16 }}>Get My Free Demo</Link>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          {tab === 'monthly' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 20 }}>
              {MONTHLY.map((s, i) => (
                <motion.div key={s.title} {...fadeUp(i * 0.07)} whileHover={{ y: -5 }} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 26, transition: 'all .3s' }}>
                  ...
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Link to="/contact" style={{ ...GS, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: '#fff', fontSize: 12, fontWeight: 700, padding: '8px 16px', borderRadius: 999 }}>Add this</Link>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          {tab === 'onetime' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 18 }}>
              {ONE_TIME.map((s, i) => (
                <motion.div key={s.title} {...fadeUp(i * 0.07)} whileHover={{ y: -4 }} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24, transition: 'all .3s' }}>
          ...
          {tab === 'bundle' && (
            <motion.div {...fadeUp()} style={{ background: 'linear-gradient(135deg,rgba(0,200,150,0.08),rgba(59,111,240,0.08))', border: '2px solid rgba(0,200,150,0.2)', borderRadius: 24, padding: 'clamp(36px,5vw,56px)', textAlign: 'center' }}>
              ...
              <div style={{ ...GS, fontSize: 68, fontWeight: 900, color: '#34d399', letterSpacing: '-4px', lineHeight: 1 }}><sup style={{ fontSize: 28, verticalAlign: 'super', letterSpacing: 0 }}>$</sup>799</div>
              <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', marginBottom: 28 }}>You save $176</div>
              <Link to="/contact" style={{ ...GS, background: 'linear-gradient(135deg,#1b6fff,#00c896)', color: '#fff', fontWeight: 700, fontSize: 15, padding: '14px 36px', borderRadius: 999, display: 'inline-block' }}>Get the Full Bundle</Link>
            </motion.div>
          )}
        </div>
      </section>
      ...
      <section style={{ padding: '80px 5%', background: '#03051a', textAlign: 'center' }}>
        <motion.div {...fadeUp()} style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <Link to="/contact" style={{ ...GS, background: 'linear-gradient(135deg,#1b6fff,#00c896)', color: '#fff', fontWeight: 700, fontSize: 15, padding: '14px 36px', borderRadius: 999, display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 24px rgba(27,111,255,0.3)' }}>Get a Free Audit <ArrowRight size={15}/></Link>
        </motion.div>
      </section>
    </div>
  )
