...
`,c=({children:e,className:t=``,style:n,onClick:c,glowColor:l=`blue`,size:u=`md`,width:d,height:f,customSize:p=!1})=>{let m=(0,r.useRef)(null);(0,r.useEffect)(()=>{let e=e=>{m.current&&(m.current.style.setProperty(`--x`,e.clientX.toFixed(2)),m.current.style.setProperty(`--xp`,(e.clientX/window.innerWidth).toFixed(2)),m.current.style.setProperty(`--y`,e.clientY.toFixed(2)),m.current.style.setProperty(`--yp`,(e.clientY/window.innerHeight).toFixed(2)))};return document.addEventListener(`pointermove`,e),()=>document.removeEventListener(`pointermove`,e)},[]);let{base:h,spread:g}=a[l],_={"--base":h,"--spread":g,"--radius":`14`,"--border":`2`,"--backdrop":`hsl(220 25% 12% / 0.08)`,"--backup-border":`var(--backdrop)`,"--size":`220`,"--outer":`1`,"--border-size":`calc(var(--border, 2) * 1px)`,"--spotlight-size":`calc(var(--size, 150) * 1px)`,"--hue":`calc(var(--base) + (var(--xp, 0) * var(--spread, 0)))`,backgroundImage:`radial-gradient(
